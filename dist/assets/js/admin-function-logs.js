...
(async function(){
  ...
  async function authHeaders(){
    ...
    const user = window.netlifyIdentity.currentUser(); if(!user) return {}; const token = await user.jwt(); return { Authorization: `Bearer ${token}` };
  }
  async function load(){
    const fn = document.getElementById('fnFilter').value.trim(); const limit = document.getElementById('limitInput').value || '200';
    ...
    if(!res.ok){ bodyEl.innerHTML = `<tr><td colspan=6 class='px-3 py-2 text-red-600'>${res.status} ${res.statusText}</td></tr>`; return; }
  ...
  if(window.netlifyIdentity){ window.netlifyIdentity.on('init', load); window.netlifyIdentity.on('login', load); }
  load();
})();
