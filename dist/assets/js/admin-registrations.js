...
(function(){
  ...
  function maskEmail(e){
    if(!masked) return e; const parts = e.split('@'); if(parts.length<2) return e; return parts[0][0] + '***@' + parts[1];
  }
  function buildUrl(base){
    const params = new URLSearchParams();
    const slug = slugInput.value.trim(); if(slug) params.set('slug', slug);
  ...
  function filterDate(created){
    const start = startInput.value; const end = endInput.value; if(!start && !end) return true;
  ...
  async function load(){
    ...
    try {
      ...
      tbody.innerHTML='';
      data.forEach(r => { if(filterDate(r.created_at)){ const tr = document.createElement('tr'); tr.className='border-b last:border-b-0'; tr.innerHTML = `<td class='py-1 pr-4 font-mono text-xs'>${r.lesson_slug}</td><td class='py-1 pr-4'>${r.name}</td><td class='py-1 pr-4 email-cell' data-email='${r.email}'>${maskEmail(r.email)}</td><td class='py-1 pr-4'>${new Date(r.created_at).toLocaleString()}</td>`; tbody.appendChild(tr);} });
  ...
  function toggleMask(){ masked=!masked; document.querySelectorAll('.email-cell').forEach(td=>{ td.textContent = masked?maskEmail(td.dataset.email):td.dataset.email; }); document.getElementById('toggle-mask').textContent = masked? 'Unmask':'Mask'; }
  function autoRefreshToggle(){ if(timer){clearInterval(timer); timer=null;} if(autoChk.checked){ timer=setInterval(load,30000);} }
